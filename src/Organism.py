...
class Organism:
    ...
    STIMULI = [
        ...
        lambda organism: (organism.x / (organism.env.width - 1)) * 2,
        lambda organism: (organism.x / (organism.env.width - 1) - 1) * 2,
        lambda organism: (organism.y / (organism.env.height - 1)) * 2,
        lambda organism: (organism.y / (organism.env.height - 1) - 1) * 2,
    ...
    def mutate(self, chance):
        ...
        if random.random() <= chance:
            ...
            self.genome[target_gene] = bytes(self.genome[target_gene][:target_byte]) + bytes([new_byte]) + bytes(self.genome[target_gene][target_byte + 1:])
